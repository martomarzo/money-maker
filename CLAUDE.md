...
- Next: user registers + creates household, FX backfill against live DB (runs on the server: throwaway node container on `docker_default` with `--env-file /opt/money-maker/.env`), first real imports at `/import`. The app container bind-mounts `/opt/money-maker/imports` → `/app/data/imports` (ro) so `/import` can see the extracted JSON — copy it there with `scp -r data/imports/extracted root@docker:/opt/money-maker/imports/` (the permission classifier blocks Claude from doing this; user must run it).
